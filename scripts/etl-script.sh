...
mkdir rawdata/$1
touch rawdata/$1/blockrange-${rangeArr[0]}-${rangeArr[1]}.csv
ethereumetl export_blocks_and_transactions --start-block ${rangeArr[0]} --end-block ${rangeArr[1]} --transactions-output rawdata/$1/transactions.csv --provider-uri https://bsc-dataseed.binance.org/ --max-workers 5 --batch-size 10 
